...
// POST: 新規作成（active・description対応）
app.post('/api/surveys', async (req, res) => {
    const token = req.headers['x-admin-token'];
    if (!token || !validTokens.has(token)) return res.status(401).json({ error: '認証が必要です' });
    const { tenant, title, description, questions, active } = req.body;
...
// PUT: 更新（active・description対応）
app.put('/api/surveys', async (req, res) => {
    const token = req.headers['x-admin-token'];
    if (!token || !validTokens.has(token)) return res.status(401).json({ error: '認証が必要です' });
    const { id, tenant, title, description, questions, active } = req.body;
