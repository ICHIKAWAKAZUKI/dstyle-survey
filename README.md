...
| ⑥ | 管理画面から受付期間を設定 → Functions（`/api/period`）→ Cosmos DB に保存 |
...
```
survey-fitting.html → survey-shinjuku.html
admin-fitting.html  → admin-shinjuku.html
```
...
例: diana-fitting             → ADMIN_PASSWORD_DIANA_FITTING
```
...
### STEP 5: 受付期間を設定する（期間制限ありの場合のみ）

管理画面にログイン後、「📅 アンケート受付期間の設定」パネルから開始日・終了日を設定してください。
...
├── survey-herbelle.html            # Herbelleアンケート画面
├── admin-fitting.html              # 補正下着試着管理画面（期間設定機能付き）
├── survey-fitting.html             # 補正下着試着アンケート画面（期間制限対応）
